    ...
    for idx_mdot, md in enumerate(mdot):
        ...
        for idx_v, vin in enumerate(vinfall):
            ...
            for idx_incl, inc in enumerate(incl):
                ...
                jcenter = int(len(im_scat.y)//2)+5  # 3 au above the midplane
                fig, ax = plt.subplots()
                ...
                ax.plot([0, 0], [-20, 20], 'w:')
                ax.plot([-30, 30], [0, 0], 'w:')
                plt.savefig(f'./Figures/pv/with_snowline/above_midplane/scattering/incl_{inc}_vinfall_{vin}_amax_{a}_mdot_{idx_mdot+5}.png')
                plt.close()
                ...
                ax.plot([0, 0], [-20, 20], 'w:')
                ax.plot([-30, 30], [0, 0], 'w:')
                plt.savefig(f'./Figures/pv/with_snowline/above_midplane/noline/incl_{inc}_vinfall_{vin}_amax_{a}_mdot_{idx_mdot+5}.png')
                plt.close()
                ...
                ax.plot([0, 0], [-20, 20], 'w:')
                ax.plot([-30, 30], [0, 0], 'w:')
                plt.savefig(f'./Figures/pv/with_snowline/above_midplane/extracted_gas/incl_{inc}_vinfall_{vin}_amax_{a}_mdot_{idx_mdot+5}.png')
                plt.close()
