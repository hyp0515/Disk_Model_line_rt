...
def sed_model(theta):
    ...
    os.system(f'radmc3d spectrum incl {incl} lambdarange 1200. 1300. nlam 100 noline noscat')
...
def log_prior(theta):
    ...
    if -3 < a_max < 2 and 0.1 < Mstar < 5 and -8 < Mdot < -6 and 50 < incl < 90 and 20 < r < 100:
        return 0.0
    return -np.inf
...
def posterior(sampler, label = ['log($a_{max}$)','10*$M_{*}$', 'log($\dot{M}$)', '$i^{\circ}$', 'Radius [au]']):
    samples = sampler.flatchain
    fig = corner.corner(
...
nwalkers, ndim = 100, 5  # Number of walkers and dimension of the parameter space
pos = [np.array([-1, 1.4, -7, 70, 60]) + 5e-1 * np.random.randn(ndim) for i in range(nwalkers)]
sampler, pos, prob, state = main(pos,nwalkers,20,ndim,log_probability, (observed_Freq, observed_Flux, err))
plotter(sampler)
posterior(sampler)
