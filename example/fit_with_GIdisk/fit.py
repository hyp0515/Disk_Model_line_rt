...
# a_array = np.linspace(-3, 1, 30, endpoint=True)
...
#             log_likelihood += -0.5 * np.sum((images[i].img - images[i].img_model) ** 2 / sigma_list[i]**2)
...
#         ax = axes[i]
#         ax.plot(a_array, np.array(parms_list[i]), color=c[idx], label=legends[idx])
#         if i == 0:
...
opacity_table = generate_opacity_table(
    a_min=1e-6, a_max=.05, # min/max grain size
    q=-3.5, # slope for dust size distribution, dn/da ~ a^q
    dust_to_gas=0.01 # dust-to-gas ratio before sublimation
)
disk_property_table = generate_disk_property_table(opacity_table = opacity_table)
...
for col in range(len(images)):
    ...
    ax[0, col].contour(DI_alma.img, levels=[50*40e-6], colors='black', linewidths=0.65)
