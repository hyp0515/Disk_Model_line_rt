...
flat_samples = reader.get_chain(flat=True, discard=100)
# flat_samples = reader.get_chain(flat=True, discard=100)
chains = reader.get_chain(discard=2000, flat=False)
...
acceptance_fractions = np.mean(
    np.diff(reader.get_log_prob(discard=2000, flat=False), axis=0) != 0, axis=0
)
...
flat_samples = filtered_chains.reshape(-1, n_params)
