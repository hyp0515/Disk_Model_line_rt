...
def GIdisk(theta):
    ...
    for i, image_class in enumerate(images):
        ...
        # print("pass")
    # D.fit(Mstar=mstar*Msun, Mdot=10**(mdot)*(Msun/yr))
    
    # ll = 0
    # for i, image_class in enumerate(images):
    #     ll += - 0.5 * np.sum((image_class.img - image_class.img_model)**2 / sigma_list[i]**2)
    
    ll = -D.fit(Mstar=mstar*Msun, Mdot=10**(mdot)*(Msun/yr))
...
def log_prior(theta):
    amax, mstar, mdot = theta
    if -2 < amax < 2 and 0.1 < mstar < 0.2 and np.log10(1e-7) < mdot < np.log10(10e-7):
        return 0.0
    return -np.inf
...
pos = [np.array([-0.0, 0.15, np.log10(5e-7)]) + [1e-2, 1e-3, 1e-2] * np.random.randn(ndim) for i in range(nwalkers)]
