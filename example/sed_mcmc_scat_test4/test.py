...
def sed_model(theta):
    ...
    lam12    = np.logspace(np.log10(lam1),np.log10(lam2),3,endpoint=False)
    lam23    = np.logspace(np.log10(lam2),np.log10(lam3),2,endpoint=True)
    lam      = np.concatenate([lam12,lam23])
    nlam     = lam.size
...
def log_likelihood(theta, Freq, Flux, err):
    ...
    mapping = {95.0: 0, 233.8: 3, 246.7: 4}
...
with Pool(n_processes) as pool:
    sampler = emcee.EnsembleSampler(nwalkers, ndim, log_probability, args=(observed_Freq, observed_Flux, err), pool=pool, backend=backend, moves=moves)
