...
class radmc3d_setup:
    ...
    Example:
      ...
      test.get_mastercontrol(filename = 'radmctest.inp',
                             comment = 'this is a test', incl_dust=1, a=1.0, b=2.0, c=3.0)
      test.get_linecontrol(line1='ch3oh leiden 0 0', 
                           line2='co leiden 0 0')
    ...
    def get_mastercontrol(self, filename = None,
      ...
      if filename != None:
        ...
        os.system('cp -r radmc3d.inp {}'.format(filename))
        f = open(filename, 'r+')
        content = f.read()
        f.seek(0,0)
        f.write('# created: ' + self.now + '\n')
        if comment != None:
          f.write('# ' + comment + '\n')
        f.write(content)
        f.close()
    ...
    def get_linecontrol(self, filename = None,
      ...
      if filename != None:
        os.system('rm -rf {}'.format(filename))
        os.system('cp -r lines.inp {}'.format(filename))
    ...
    def write_dust_density(self):
      ...
      if self.disk_boundary is not None: # setting the boundary of the disk
        self.rho_dust = self.d_to_gas_ratio * np.where(np.log10(self.DM.rho_sph)<np.log10(self.disk_boundary),
                                                  self.disk_boundary,
                                                  self.DM.rho_sph
                                                  ) 
    ...
    def get_heatcontrol(self, L_star = None,
      ...
      wavelength_micron.inp, which is for continuum calculation, is also generated in this function .
      ...
      lam3     = 5.0e3
      lam4     = 1.0e4
      n12      = 100  # this section is important when irradiation heating since it covers the peak of the blackbody emission of the central star.
      ...
      if irradiation is True:# Irradiation heating calculated by RADMC-3D
        if accretion is False:
          if self.disk_boundary is not None:
            ...
            T = np.where(np.tile(self.rho_dust[:, :, :, np.newaxis], (1, 1, 1, 4)) < self.disk_boundary*self.d_to_gas_ratio,
                         20,
                         d.dusttemp
                         )  # setting disk boundary
            T = np.where(d.dusttemp < 20, 20, d.dusttemp) # setting the minimum temperature to maintain consistency and prevernt 0 K
        ...
        if self.disk_boundary is not None:
          T = np.where(self.rho_dust< self.disk_boundary * self.d_to_gas_ratio,
                       20,
                       self.DM.T_sph
                       )
