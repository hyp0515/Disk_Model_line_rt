...
plot_polar_mesh(grid.x/au, grid.y, grid.z, np.log10(nch3oh[:, :, 0]), 
                'Number density map of methanol', r'log($n_{\mathregular{CH_3OH}}$) [$cm^{-3}$]',"ndensity_profile_mc",'BuPu', type='edge')
plot_polar_mesh(grid.x/au, grid.y, grid.z, np.log10(data.dusttemp[:, :, 0, 0]), 
                'Temperature map', r'log(T) [K]', "T_profile_mc", 'OrRd', type='edge')
plot_polar_mesh(grid.x/au, grid.y, grid.z, np.log10(data.rhodust[:, :, 0, 0]), 
                'Dust density map', r'log($\rho$) [g$cm^{-3}$]', "rho_profile_mc",'BuPu', type='edge')
