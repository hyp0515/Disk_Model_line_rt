...
# os.system(f"radmc3d spectrum incl 0 phi 0 iline 1 widthkms 100 vkms 0 linenlam 100")
# s = readSpectrum('spectrum.out')
# peak_lam = s[np.argmax(s, axis=1),0]
# v_axis = cc*((s[:,0]**2/peak_lam**2-1)/(s[:,0]**2/peak_lam**2+1))*1e-5
# plt.plot(v_axis, 1e26*s[:,1], label=f'0'+r'$^\circ$')
...
#     os.system(f"radmc3d spectrum incl {deg} phi 0 iline 1 widthkms 100 vkms 0 linenlam 100")
#     s = readSpectrum('spectrum.out')
#     plt.plot(v_axis, 1e26*s[:,1], label=f'{deg}'+r'$^\circ$')
# plt.legend()
# plt.xlabel('Velocity (km/s)')
# plt.ylabel('Intensity (mJy/beam)')
...
problem_setup(a_max=0.1, Mass_of_star=0.14*Msun, Accretion_rate=0.14e-5*Msun/yr, Radius_of_disk=30*au, pancake=False)
v_list = np.linspace(-1, 1, 11, endpoint=True)
