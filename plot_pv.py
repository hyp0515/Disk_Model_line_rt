...
for idx_v, vin in enumerate(vinfall):
    ...
    for idx_incl, inc in enumerate(incl):
        ...
        os.system(f"radmc3d image iline 240 incl {inc} vkms 0 widthkms 20 linenlam 101 npix 100 nodust")
        im = readImage()
        freq0 = im.freq[50]
        v = cc / 1e5 * (freq0 - im.freq) / freq0
        # print(im.wav)
        ...
        plt.ylabel("Velocity [km/s]")
        # plt.show()
        plt.savefig(f'./Figures/pv/width_20/no_dust/incl_{inc}_vinfall_{vin:.1f}.png')
        plt.close()
...
for idx_v, vin in enumerate(vinfall):
    ...
    for idx_incl, inc in enumerate(incl):
        ...
        os.system(f"radmc3d image iline 240 incl {inc} vkms 0 widthkms 20 linenlam 101 npix 100")
        im = readImage()
        freq0 = im.freq[50]
        v = cc / 1e5 * (freq0 - im.freq) / freq0
        ...
        plt.ylabel("Velocity [km/s]")
        # plt.show()
        plt.savefig(f'./Figures/pv/width_20/include_dust/incl_{inc}_vinfall_{vin:.1f}.png')
        plt.close()