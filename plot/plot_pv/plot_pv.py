...
def plot_pv(incl=70, line=240, vkm=0, v_width=20, nlam=50,
    ...
    if convolve is not True: 
        fig, ax = plt.subplots() 
        c = ax.pcolormesh(im.x/au, v+vkm, data[:, center, :].T, shading="nearest", rasterized=True, cmap='jet')
    ...
    for idx_h, heat in enumerate(heat_list):
        for idx_s, snow in enumerate(snowline):
            if heat == 'Accretion' and snow =='w/o snowline':
                problem_setup(a_max=amax, Mass_of_star=0.14*Msun, Accretion_rate=0.14e-5*Msun/yr, Radius_of_disk=100*au, v_infall=1, 
            ...
            for idx_d, d in enumerate(dust):
                if convolve is not True:
                    ...
                    if d == 'w/o dust':
                        fig, ax = plot_pv(incl=70, vkm=v0, v_width=vwidth, nlam=nlam, nodust=True, npix=npix, sizeau=sizeau, convolve=convolve, fwhm=fwhm)
                        title = t + ' + w/o dust'
                        fname = f + ' + wo dust'
                    elif d == 'w/ dust':
                        fig, ax = plot_pv(incl=70, vkm=v0, v_width=vwidth, nlam=nlam, extract_gas=True, npix=npix, sizeau=sizeau, convolve=convolve, fwhm=fwhm)
                        title = t + ' + w/ dust'
                        fname = f + ' + w dust'
...
for idx_a, a in enumerate([0.1, 0.01, 0.001]):
    multiple_plots(amax=a, rcb=30, nlam=100, npix=300, sizeau=300, v0=5, vwidth=10, gas_inside=True, convolve=True, fwhm=50)
